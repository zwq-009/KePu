<template>
	<view class="center">
		<view class="logo" @click="goDetail">
			<image class="logo-img" :src="user.avatarUrl"></image>
			<view class="logo-title">
				<text class="uer-name" @click="goDetail()">Hi，{{user.name}}</text>
	...
	export default {
		...
		onShow() {
			...
			if (userinfo) {
				...
				this.user.avatarUrl = this.userinfo.avatarUrl
			} else if (user) {
				console.log(user[0].name)
				this.user.name = user[0].name
				this.user.avatarUrl = this.user.avatarUrl1
			}
			//#endif
			var userr = uni.getStorageSync("user")
			this.user.name = userr[0].name
			this.user.avatarUrl = this.user.avatarUrl1
		...
		methods: {
			goDetail() {
				uni.navigateTo({
					url: '/pages/QuShe-login/QuShe-login'
				});
			},
			none() {
				uni.navigateTo({
